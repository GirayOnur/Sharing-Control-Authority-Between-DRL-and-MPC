...
%
% Saves: dw_max_multi.svg  +  dw_max_multi_table.tex
...
for s = 1:n_scen
    ...
    ylabel(ax, '$\Delta w_{\max}$ (veh)', 'FontSize', ax_fsize, 'Interpreter', 'latex');
...
print(fig, fullfile(out_dir, 'dw_max_multi.svg'), '-dsvg', '-vector');
fprintf('Saved figure → dw_max_multi.svg\n');

%% ── LaTeX table ────────────────────────────────────────────────────────────
fid = fopen(fullfile(out_dir, 'dw_max_multi_table.tex'), 'w');
fprintf(fid, '\\begin{table}[htbp]\n\\centering\n');
fprintf(fid, ['\\caption{Mean $\\pm$ standard deviation of $\\Delta w_{\\max}$ (veh) ' ...
    'for PI-ALINEA, DDPG-MPC, SAC-MPC, and SAC(D)-MPC across all 5 training runs.}\n']);
fprintf(fid, '\\label{tab:dw_max_multi}\n');
...
fclose(fid);
fprintf('Saved table   → dw_max_multi_table.tex\n');
...
fprintf('%-12s  %-12s  %-28s  %-28s\n', ...
        'Algorithm', 'Scenario', 'Median agent (dw_max)', 'Best agent (dw_max)');
fprintf('%s\n', repmat('-', 1, 70));
for c = 1:n_ctrl
    for s = 1:n_scen
        v    = dw_data{c,s};
