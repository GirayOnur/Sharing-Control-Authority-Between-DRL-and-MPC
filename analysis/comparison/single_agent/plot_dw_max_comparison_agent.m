%% plot_dw_max_comparison_agent.m
%
% Agent-indexed version of plot_dw_max_comparison.m.
...
%
% Saves: dw_max_comparison_agent.svg  +  dw_max_table_agent.tex
...
out_fig = fullfile(out_dir, 'dw_max_comparison_agent.svg');
print(fig, out_fig, '-dsvg', '-vector');
fprintf('Saved figure → %s\n', out_fig);

%% ── LaTeX table ────────────────────────────────────────────────────────────
tex_file = fullfile(out_dir, 'dw_max_table_agent.tex');
...
fprintf(fid, ['\\caption{Mean $\\pm$ standard deviation of maximum queue ' ...
    ...
    'PI-ALINEA uses agent~%s; DDPG-MPC uses agent~%s; SAC-MPC uses agent~%s; SAC(D)-MPC uses agent~%s.}\n'], mat2str(pi_alinea_agent), mat2str(ddpg_agent), mat2str(sac_agent), mat2str(sacd_agent));
fprintf(fid, '\\label{tab:dw_max_comparison_agent}\n');
