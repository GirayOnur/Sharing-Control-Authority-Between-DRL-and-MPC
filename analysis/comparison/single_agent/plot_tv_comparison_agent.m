%% plot_tv_comparison_agent.m
%
% Agent-indexed version of plot_tv_comparison.m.
...
%
% TV = Σ_{k=2}^{N} ||u(k) − u(k−1)||_2   where u(k) ∈ R^3
%
% Saves: tv_comparison_agent.svg  +  tv_table_agent.tex
...
out_fig = fullfile(out_dir, 'tv_comparison_agent.svg');
print(fig, out_fig, '-dsvg', '-vector');
fprintf('Saved figure → %s\n', out_fig);

%% ── LaTeX table ────────────────────────────────────────────────────────────
tex_file = fullfile(out_dir, 'tv_table_agent.tex');
...
fprintf(fid, '\\label{tab:tv_comparison_agent}\n');
