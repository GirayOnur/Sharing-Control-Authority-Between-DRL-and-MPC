%% plot_obj_comparison_agent.m
%
% Agent-indexed version of plot_obj_comparison.m.
...
%   obj = u_pen + tts + q_pen
%
% Saves: obj_comparison_agent.svg  +  obj_table_agent.tex
...
M_low  = 6;      % low-level control period [sim steps]
...
%% ── Helper: compute objective cost from raw xx, uu, param_sim ─────────────
...
out_fig = fullfile(out_dir, 'obj_comparison_agent.svg');
print(fig, out_fig, '-dsvg', '-vector');
fprintf('Saved figure → %s\n', out_fig);

%% ── LaTeX table ────────────────────────────────────────────────────────────
tex_file = fullfile(out_dir, 'obj_table_agent.tex');
fid      = fopen(tex_file, 'w');
fprintf(fid, '\\begin{table}[htbp]\n\\centering\n');
fprintf(fid, ['\\caption{Mean $\\pm$ standard deviation of the overall ' ...
    'objective cost ($J = u_{\\mathrm{pen}} + \\mathrm{TTS} + q_{\\mathrm{pen}}$) ' ...
    'across all experimental runs. PI-ALINEA uses agent~%s; DDPG-MPC uses agent~%s; SAC-MPC uses agent~%s; SAC(D)-MPC uses agent~%s.}\n'], ...
    mat2str(pi_alinea_agent), mat2str(ddpg_agent), mat2str(sac_agent), mat2str(sacd_agent));
fprintf(fid, '\\label{tab:obj_comparison_agent}\n');
