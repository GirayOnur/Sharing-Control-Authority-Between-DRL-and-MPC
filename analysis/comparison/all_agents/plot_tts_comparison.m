...
%   6. SAC(D)-MPC       - deterministic SAC-MPC (5 runs x 10 experiments)
%
% Scenarios (4):
...
%   • DDPG/SAC/SAC(D) pooled across 5 seeds → spread reflects both training and
...
for s = 1:n_scen
    ...
    % ── 6. SAC(D)-MPC (5 run folders × 10 experiments) ───────────────────────
    tts_v = []; qv = []; ridx = [];
    for r = 1:n_runs
...
function fig = make_boxplot_figure(tts_data, ctrl_clr, ctrl_labels, ...
    ...
    for s = 1:n_scen
        ...
        if det_nc
            ...
            ctrl_idx = 2:n_ctrl;            % controllers 2-6
        else
            % No Control box included — no dashed line
            hl       = [];
            ctrl_idx = 1:n_ctrl;            % all 6 controllers
        end
...
fig = make_boxplot_figure(tts_data, ctrl_clr, ctrl_labels, ...
                          scen_labels, y_lo, y_hi, ax_fsize, ttl_fsize);
