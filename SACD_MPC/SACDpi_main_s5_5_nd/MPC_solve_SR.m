function u_mpc = MPC_solve_SR(x0,u0,u_sr_prev,u_rm,k,param,MPC_param_low,MPC_param_high,scenario,demands)
...
fun = @(u_mpc) J_calc(x0, u_mpc, k, Nc, Np, M, u_sr_prev, u_rm,Nc_high, Np_high, M_high, r_cost, s_cost, param, scenario,demands);
nonlcon = @(u_mpc) x_con(x0, u_mpc, k, w_con, Nc, Np, M,u_rm, Nc_high, Np_high, M_high, param, scenario,demands);
...
%%%
function J = J_calc(x,u,k,Nc,Np,M,u0_1,u_rm,Nc_high,Np_high,M_high,r_cost,s_cost,param,scenario,demands)
    k_obj = k;
    u1 = [u0_1, u];
...
function [c,ceq] = x_con(x,u,k,w_con,Nc,Np,M,u_rm,Nc_high,Np_high,M_high,param,scenario,demands)
    ceq = [];
    %c = [];
