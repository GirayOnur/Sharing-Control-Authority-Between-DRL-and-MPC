...
for i=1:N
    if mod(k_c,param_MPC_low.M) == 0
        tic
        u_mpc_rm = MPC_solve_RM_mm(x,u_mpc_rm_0,u_mpc_rm_prev,u_mpc_sr,k,param_esti,param_MPC_low,param_MPC_high,scenario);
    ...
    if mod(k_c,param_MPC_high.M) == 0
        tic
        u_mpc_sr = MPC_solve_SR_mm(x,u_mpc_sr_0,u_mpc_sr_prev,u_mpc_rm,k,param_esti,param_MPC_low,param_MPC_high,scenario);
        total_comp_time = total_comp_time + toc;
        u_mpc_sr_0 = [u_mpc_sr(:,2:end),u_mpc_sr(:,end)];
        u_mpc_sr_prev = u_mpc_sr(:,1);
    end

    x = fun_benchmark_RM(x,[u_mpc_sr(:,1);u_mpc_rm(:,1)],k,param_sim,scenario);
    xx(:,i) = x;
...
mpc_result_doc_name = 'MPC_RM_SR_hier_mm_result_' + string(datetime('now'), 'yyyy-MM-dd hh_mm_ss') + '.mat';
