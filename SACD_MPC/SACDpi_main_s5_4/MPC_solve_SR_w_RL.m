function u_mpc = MPC_solve_SR_w_RL(x0,u0,u_mpc_prev,u_rm,k,param,MPC_param_low,MPC_param_high,scenario,agent)
...
%
% Soft constrained version: the queue limits enter the cost as q_pen. This
% is the one used during training (rlStepFunc), where a failed solve would
% stall the episode. MPC_solve_SR_w_RL_h is the same thing with the limits
% as hard constraints and is what the benchmark runs use.
...
function J = J_calc(x,u,k,Nc,Np,M,u0_1,u_rm,Nc_high,Np_high,M_high,r_cost,s_cost,param,scenario,agent,x_norm)
    ...
    for i=1:Np*M
        ...
        if mod(k_obj,M) == 0
            [demando1c1,demando1c2]  = demando1(k_obj-1,scenario);
            [demando2c1,demando2c2]  = demando2(k_obj-1,scenario);
            [demando3c1,demando3c2]  = demando3(k_obj-1,scenario);
            agentObs = [x./x_norm;...
        ...
        x = fun_benchmark_RM(x,[u_seq_high(:,i);u_rl],k_obj,param,scenario);
        x_seq(:,i) = x;
        k_obj = k_obj + 1;
    end
...
function [c,ceq] = x_con(x,u,k,w_con,Nc,Np,M,u_rm,Nc_high,Np_high,M_high,param,scenario,agent,x_norm)
    ...
    % 
    %     if mod(k_con,M) == 0
    %         [demando1c1,demando1c2]  = demando1(k_con-1,scenario);
    %         [demando2c1,demando2c2]  = demando2(k_con-1,scenario);
    %         [demando3c1,demando3c2]  = demando3(k_con-1,scenario);
    %         agentObs = [x./x_norm;...
    %                      [demando1c1,demando1c2]'./1000;...
    %                      [demando2c1,demando2c2]'./1000;...
    %                      [demando3c1,demando3c2]'./1000];       
    ...
    %     end
    % 
    %     x = fun_benchmark_RM(x,[u_seq_high(:,i);u_rl],k_con,param,scenario);
    %     x_seq(:,i) = x;
    %     k_con = k_con + 1;
    % end
    % 
