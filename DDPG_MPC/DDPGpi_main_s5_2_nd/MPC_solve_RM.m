function u_mpc = MPC_solve_RM(x0,u0,u_rm_prev,u_sr,k,param,MPC_param_low,MPC_param_high,scenario,demands)
% Low-level MPC controller of the hierarchical MPC framework: computes the
% two ramp metering rates every T_l = 60 s over a prediction horizon of
% Np = 10 low-level steps.
%
% In its prediction, the vehicle split rate is taken from the most recent
% high-level trajectory (u_sr), shifted one step with the terminal value
% duplicated, so this level optimizes the ramp metering rates only.
...
lb = repmat([0;0],1,Nc);  %one metering rate per on-ramp per control step
ub = repmat([1;1],1,Nc);

fun = @(u_mpc) J_calc(x0, u_mpc, k, Nc, Np, M, u_rm_prev, u_sr,Nc_high, Np_high, M_high, r_cost, s_cost, param, scenario,demands);
nonlcon = @(u_mpc) x_con(x0, u_mpc, k, w_con, Nc, Np, M,u_sr, Nc_high, Np_high, M_high, param, scenario,demands);
...
%%%
function J = J_calc(x,u,k,Nc,Np,M,u0_1,u_sr,Nc_high,Np_high,M_high,r_cost,s_cost,param,scenario,demands)
    k_obj = k;
    u1 = [u0_1, u];
    ...
    for i=1:Np*M
        x = fun_benchmark_RM_nd(x,u_seq(:,i),k_obj,param,scenario,demands);
        x_seq(:,i) = x;
        k_obj = k_obj + 1;
    end
...
function [c,ceq] = x_con(x,u,k,w_con,Nc,Np,M,u_sr,Nc_high,Np_high,M_high,param,scenario,demands)
    ceq = [];
    %c = [];
    ...
    x_seq = zeros(size(x,1),Np*M);
    for i=1:Np*M
        x = fun_benchmark_RM_nd(x,u_seq(:,i),k_con,param,scenario,demands);
        x_seq(:,i) = x;
        k_con = k_con + 1;
    end
