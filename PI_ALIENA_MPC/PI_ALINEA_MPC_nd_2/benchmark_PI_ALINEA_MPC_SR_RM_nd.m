...
load("Bayes_opt_PI_ALINEA_nd_result_2025-09-08 08_24_13.mat")
...
for i=1:N
    ...
    if mod(k_c,param_MPC_high.M) == 0
        tic
        u_mpc_sr = MPC_solve_SR_w_PI_ALINEA_h_nd(x,u_mpc_sr_0,u_mpc_sr_prev,u_PI_ALINEA,k,param_sim,param_MPC_low,param_MPC_high,scenario,pi_alinea_params,x_prev,u_PI_ALINEA_prev,Demands);
        total_comp_time = total_comp_time + toc;
        u_mpc_sr_0 = [u_mpc_sr(:,2:end),u_mpc_sr(:,end)];
        u_mpc_sr_prev = u_mpc_sr(:,1);
    end

    x = fun_benchmark_RM_nd(x,[u_mpc_sr(:,1);u_PI_ALINEA],k,param_sim,scenario,Demands);
    xx(:,i) = x;
...
mpc_result_doc_name = 'PI_ALINEA_MPC_SR_RM_nd_result_' + string(datetime('now'), 'yyyy-MM-dd hh_mm_ss') + '.mat';
