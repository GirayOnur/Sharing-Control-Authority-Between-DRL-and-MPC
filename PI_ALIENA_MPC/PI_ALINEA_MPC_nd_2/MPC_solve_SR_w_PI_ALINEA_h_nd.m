function u_mpc = MPC_solve_SR_w_PI_ALINEA_h_nd(x0,u0,u_mpc_prev,u_rm,k,param,MPC_param_low,MPC_param_high,scenario,pi_alinea_params,x_prev,u_PI_ALINEA,demands)
% High-level MPC controller of the state-feedback-MPC (SF-MPC) framework:
% computes the vehicle split rate. Same idea as the DRL-MPC version, with
% the PI-ALINEA law stepped inside the prediction instead of the DRL policy,
% so the predicted ramp metering rates match what the low-level controller
% would apply. The _h variant enforces the queue length limits as hard
% constraints, the other one as a penalty in the cost.
...
fun = @(u_mpc) J_calc(x0, u_mpc, k, Nc, Np, M, u_mpc_prev, u_rm,Nc_high, Np_high, M_high, r_cost, s_cost, param, scenario,pi_alinea_params,x_prev,u_PI_ALINEA,demands);
%nonlcon = [];
nonlcon = @(u_mpc) x_con(x0, u_mpc, k, w_con, Nc, Np, M,u_rm, Nc_high, Np_high, M_high, param, scenario,pi_alinea_params,x_prev,u_PI_ALINEA,demands);
...
%%%
function J = J_calc(x,u,k,Nc,Np,M,u0_1,u_rm,Nc_high,Np_high,M_high,r_cost,s_cost,param,scenario,pi_alinea_params,x_prev,u_PI_ALINEA,demands)
    k_obj = k;
    u1 = [u0_1, u];
    ...
    for i=1:Np*M
        ...
        x = fun_benchmark_RM_nd(x,[u_seq_high(:,i);u_PI_ALINEA],k_obj,param,scenario,demands);
        x_seq(:,i) = x;
        k_obj = k_obj + 1;
    end
...
function [c,ceq] = x_con(x,u,k,w_con,Nc,Np,M,u_rm,Nc_high,Np_high,M_high,param,scenario,pi_alinea_params,x_prev,u_PI_ALINEA,demands)
    ceq = [];
    %c = [];
    ...
    for i=1:Np*M
        ...
        x = fun_benchmark_RM_nd(x,[u_seq_high(:,i);u_PI_ALINEA],k_con,param,scenario,demands);
        x_seq(:,i) = x;
        k_con = k_con + 1;
    end
