function u_mpc = MPC_solve_SR_w_PI_ALINEA_h(x0,u0,u_mpc_prev,u_rm,k,param,MPC_param_low,MPC_param_high,scenario,pi_alinea_params,x_prev,u_PI_ALINEA)
...
function J = J_calc(x,u,k,Nc,Np,M,u0_1,u_rm,Nc_high,Np_high,M_high,r_cost,s_cost,param,scenario,pi_alinea_params,x_prev,u_PI_ALINEA)
    ...
    for i=1:Np*M
        ...
        x = fun_benchmark_RM(x,[u_seq_high(:,i);u_PI_ALINEA],k_obj,param,scenario);
        x_seq(:,i) = x;
        k_obj = k_obj + 1;
    end
...
function [c,ceq] = x_con(x,u,k,w_con,Nc,Np,M,u_rm,Nc_high,Np_high,M_high,param,scenario,pi_alinea_params,x_prev,u_PI_ALINEA)
    ...
    for i=1:Np*M
        ...
        x = fun_benchmark_RM(x,[u_seq_high(:,i);u_PI_ALINEA],k_con,param,scenario);
        x_seq(:,i) = x;
        k_con = k_con + 1;
    end
