...
% and computes the total input variation (TIV) as a scalar:
%
%   TIV = Σ_{k=2}^{N} ||u(k) − u(k−1)||_2
%
...
%   S1/S3 - No Control shown as dashed reference line (deterministic, TIV = 0)
%   S2/S4 - No Control shown as a box (stochastic, TIV ≈ 0)
%
...
%% ── Helper: compute TIV from raw uu ────────────────────────────────────────
function tv = compute_tv(uu)
    % uu : [n_inputs × N]
    % TIV = sum of L2 norms of consecutive differences
...
for s = 1:n_scen
    ...
    ylabel(ax, '$\mathrm{TIV}$', 'FontSize', ax_fsize, 'Interpreter', 'latex', 'Color', 'k');
...
out_pdf = fullfile(out_dir, 'tv_comparison.pdf');
...
fprintf(fid, ['\\caption{Mean $\\pm$ standard deviation of total input ' ...
    'variation (TIV) across all experimental runs.}\n']);
...
fprintf('\n%s\n', repmat('=', 1, 60));
fprintf('Median-representative agent (TIV)\n');
fprintf('%s\n', repmat('=', 1, 60));
fprintf('%-10s  %-12s  %-8s  %-12s  %s\n', ...
        'Algorithm', 'Scenario', 'Agent', 'TIV(med)', 'Median of all');
fprintf('%s\n', repmat('-', 1, 60));
drl_ctrl  = {2, 4, 5, 6};
