%% plot_dw_max_comparison.m
%
% Loads raw simulation result .mat files for all 6 controllers × 4 scenarios
% and computes the maximum queue constraint violation (Δw_max, veh):
%
%   Δw_o(k)   = max(0, w_o(k) − w_o^max)          per origin, per timestep
%   Δw_max    = max_{o,k} Δw_o(k)                  worst single exceedance
%
% where w_o(k) = total queue (class 1 + class 2) at origin o,
%       w_o^max = [200, 100, 100] veh  (from param.w_con).
%
...
%
% Saves: dw_max_comparison.svg  +  dw_max_table.tex
...
%% ── Helper: compute Δw_max from raw xx ────────────────────────────────────
function dw_max = compute_dw_max(xx, w_max)
    w_o1 = xx(64,:) + xx(65,:);   % total queue at O1
    w_o2 = xx(68,:) + xx(69,:);   % total queue at O2
    w_o3 = xx(72,:) + xx(73,:);   % total queue at O3

    dw_max = max([ max(max(0, w_o1 - w_max(1))), ...
                   max(max(0, w_o2 - w_max(2))), ...
                   max(max(0, w_o3 - w_max(3))) ]);
end
...
for s = 1:n_scen
    ...
    for f = 1:numel(nc_files)
        ...
        dw_v(f) = compute_dw_max(d.xx, w_max);
    end
    dw_data{1,s} = dw_v;
    ...
    for r = 1:n_runs
        ...
        for f = 1:numel(files)
            ...
            dw_v(end+1, 1) = compute_dw_max(d.xx, w_max);
            ridx(end+1, 1) = r;
        end
    end
    dw_data{2,s}      = dw_v;
...
%% ── Console summary ────────────────────────────────────────────────────────
fprintf('Δw_max summary (mean ± std, veh):\n');
fprintf('%-18s', '');
...
y_lo = -0.03 * y_hi;   % small margin below zero so zero-violation boxes are visible
...
for s = 1:n_scen
    ...
    ylabel(ax, '$\Delta w_{\max}$ (veh)', ...
           'FontSize', ax_fsize, 'Interpreter', 'latex', 'Color', 'k');
...
out_fig = fullfile(out_dir, 'dw_max_comparison.svg');
print(fig, out_fig, '-dsvg', '-vector');
fprintf('Saved figure → %s\n', out_fig);
out_pdf = fullfile(out_dir, 'dw_max_comparison.pdf');
print(fig, out_pdf, '-dpdf', '-vector');
fprintf('Saved PDF figure -> %s\n', out_pdf);

%% ── LaTeX table ────────────────────────────────────────────────────────────
tex_file = fullfile(out_dir, 'dw_max_table.tex');
fid      = fopen(tex_file, 'w');
fprintf(fid, '\\begin{table}[htbp]\n\\centering\n');
fprintf(fid, ['\\caption{Mean $\\pm$ standard deviation of maximum queue ' ...
    'constraint violation ($\\Delta w_{\\max}$, veh) across all experimental runs.}\n']);
fprintf(fid, '\\label{tab:dw_max_comparison}\n');
...
fprintf('\n%s\n', repmat('=', 1, 60));
fprintf('Median-representative agent (Δw_max)\n');
fprintf('%s\n', repmat('=', 1, 60));
fprintf('%-10s  %-12s  %-8s  %-12s  %s\n', ...
        'Algorithm', 'Scenario', 'Agent', 'dw_max(med)', 'Median of all');
fprintf('%s\n', repmat('-', 1, 60));
drl_ctrl  = {2, 4, 5, 6};
