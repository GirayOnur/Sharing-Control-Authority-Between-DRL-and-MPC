...
% benchmark scripts, so they are loaded for the summary table but omitted
% from the figure.
...
out_pdf = fullfile(out_dir, 'comp_time_comparison.pdf');
...
function fig = make_boxplot_figure(time_data, ctrl_clr, ctrl_labels, plot_ctrl_order, ...
    ...
    for s = 1:n_scen
        ...
        ylabel(ax, 'Computation time (s)', 'FontSize', ax_fsize, 'Interpreter', 'latex', 'Color', 'k');
        title(ax, scen_labels{s}, 'FontSize', ttl_fsize, 'FontWeight', 'bold', 'Interpreter', 'latex', 'Color', 'k');
