...
%
% Saves: dw_tot_comparison.svg  +  dw_tot_table.tex
...
for s = 1:n_scen
    ...
    for f = 1:numel(nc_files)
        ...
        dw_v(f) = compute_dw_tot(d.xx, w_max);
    end
    dw_data{1,s} = dw_v;
    ...
    for r = 1:n_runs
        ...
        for f = 1:numel(files)
            ...
            dw_v(end+1, 1) = compute_dw_tot(d.xx, w_max);
            ridx(end+1, 1) = r;
        end
    end
    dw_data{2,s}      = dw_v;
...
%% ── Console summary ────────────────────────────────────────────────────────
fprintf('Δw_tot summary (mean ± std, veh):\n');
fprintf('%-18s', '');
...
for s = 1:n_scen
    ...
    ylabel(ax, '$\Delta w_{\mathrm{tot}}$ (veh)', ...
...
out_fig = fullfile(out_dir, 'dw_tot_comparison.svg');
print(fig, out_fig, '-dsvg', '-vector');
fprintf('Saved figure → %s\n', out_fig);
out_pdf = fullfile(out_dir, 'dw_tot_comparison.pdf');
print(fig, out_pdf, '-dpdf', '-vector');
fprintf('Saved PDF figure -> %s\n', out_pdf);

%% ── LaTeX table ────────────────────────────────────────────────────────────
tex_file = fullfile(out_dir, 'dw_tot_table.tex');
fid      = fopen(tex_file, 'w');
fprintf(fid, '\\begin{table}[htbp]\n\\centering\n');
fprintf(fid, ['\\caption{Mean $\\pm$ standard deviation of total queue ' ...
    'constraint violation ($\\Delta w_{\\mathrm{tot}}$, veh) ' ...
    'across all experimental runs.}\n']);
fprintf(fid, '\\label{tab:dw_tot_comparison}\n');
...
fprintf('\n%s\n', repmat('=', 1, 60));
fprintf('Median-representative agent (Δw_tot)\n');
fprintf('%s\n', repmat('=', 1, 60));
fprintf('%-10s  %-12s  %-8s  %-12s  %s\n', ...
        'Algorithm', 'Scenario', 'Agent', 'dw_tot(med)', 'Median of all');
fprintf('%s\n', repmat('-', 1, 60));
drl_ctrl  = {2, 4, 5, 6};
drl_names = {'SF-MPC', 'DDPG-MPC', 'SAC-MPC', 'SACD-MPC'};

for di = 1:numel(drl_ctrl)
    c     = drl_ctrl{di};   % ctrl index: 4=DDPG, 5=SAC
    for s = 1:n_scen
        v    = dw_data{c,s};
